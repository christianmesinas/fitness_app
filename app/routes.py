...
from app.auth import bp  # Importeer het auth Blueprint
...
# Voeg context processor en before_request toe aan het Blueprint
...
def logout():
    ...
    return redirect(
        ...
        f"returnTo={url_for('auth.index', _external=True)}&client_id={current_app.config['AUTH0_CLIENT_ID']}"
    )
...
def messages():
    ...
    messages = db.paginate(query, page=page,
                           per_page=current_app.config['POSTS_PER_PAGE'], error_out=False)
    next_url = url_for('auth.messages', page=messages.next_num) if messages.has_next else None
    prev_url = url_for('auth.messages', page=messages.prev_num) if messages.has_prev else None
